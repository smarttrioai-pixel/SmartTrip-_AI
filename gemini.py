...
async def generate_json_from_image(
    *, system_prompt: str, user_prompt: str, image_bytes: bytes, mime_type: str = "image/jpeg"
) -> dict:
    """
    Multimodal call: sends the actual image alongside the prompt, unlike
    generate_json which is text-only. Landmark vision analysis requires
    this — analyzing a text hint describing "a landmark" without ever
    looking at the photo is not vision, it's guessing from a caption.
    """
    _ensure_configured()
