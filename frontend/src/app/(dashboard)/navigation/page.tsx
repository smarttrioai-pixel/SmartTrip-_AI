...
import { Navigation, MapPin, Navigation2, Volume2, Clock, Compass, AlertTriangle } from "lucide-react";
...
export default function NavigationPage() {
  ...
  const [destination, setDestination] = useState(activeTripDestination);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 relative min-h-[500px] rounded-2xl overflow-hidden border border-ink-100 dark:border-ink-700 bg-ink-900 shadow-md flex items-center justify-center text-white">
          ...
          <div className="relative z-10 flex flex-col items-center gap-4 text-center p-6 bg-ink-950/80 backdrop-blur-md rounded-2xl border border-ink-700 max-w-md">
            ...
            <h2 className="text-lg font-bold">MapLibre Dynamic Routing Canvas</h2>
            <p className="text-xs text-ink-300">
              {/* Real MapLibre rendering requires the maplibre-gl package,
                  which isn't installed yet — this placeholder no longer
                  claims tiles are loaded when none are being rendered. */}
              Map rendering not yet available (MapLibre integration pending).{" "}
              {hasRealRoute ? (
                <>
                  Real route calculated for <span className="text-brand-400 font-semibold">{destination}</span> via {travelMode}.
                </>
              ) : (
                "Search a destination to calculate a real route."
              )}
            </p>
            {geocodeData && (
              <div className="flex items-center gap-2 text-xs text-emerald-400 font-mono">
                <MapPin className="h-4 w-4" /> Coordinates: [{geocodeData.lat.toFixed(4)}, {geocodeData.lon.toFixed(4)}]
              </div>
            )}
          </div>
        </div>
