...
export function PlaceCard({
  ...
  const mealTypeLabel = (
    enrichment && ["meal", "restaurant", "foods", "food"].some(
      (value) => (enrichment.category ?? "").toLowerCase().includes(value)
    )
  )
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="font-medium text-ink-900 dark:text-white">
                {enrichment?.matchedPlaceName ?? title}
              </p>
              {enrichment?.matchedPlaceName && enrichment.matchedPlaceName !== title && (
                <p className="text-xs text-ink-400">
                  {title}{mealTypeLabel ? ` · ${mealTypeLabel}` : ""}
                </p>
              )}
            </div>
