...
import { Bookmark, BookmarkCheck, Clock, MapPin, Sparkles } from "lucide-react";
              ...
              <div key={i} className="flex gap-4 border-l-2 border-brand-100 pl-4 dark:border-brand-900/40">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-1.5 flex items-center gap-3 text-xs text-ink-400">
                    ...
                    <span>
                      ~{(activity.estimatedCost ?? 0).toLocaleString()} {trip.currency}
                    </span>
                  </div>
                  {activity.explanation && (
                    <details className="mt-2 text-xs">
                      ...
                      <div className="mt-1.5 rounded-lg bg-ink-50 p-2.5 text-ink-700 dark:bg-ink-900 dark:text-ink-100">
                        <p>{activity.explanation.reasonText}</p>
                        <p className="mt-1 text-ink-400">
                          Confidence: {Math.round(activity.explanation.confidence * 100)}%
                        </p>
                      </div>
                    </details>
                  )}
