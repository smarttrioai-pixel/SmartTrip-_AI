    ...
    <div className="mx-auto max-w-5xl px-6 py-10">
      ...
      <div className="mt-10">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-display text-lg font-semibold text-ink-900 dark:text-white">Recent trips</h2>
          <Link href="/saved-trips" className="text-sm font-medium text-brand-600 hover:underline">
            View all
          </Link>
        </div>

        {isLoading && <p className="text-sm text-ink-400">Loading your trips…</p>}
        {!isLoading && recentTrips.length === 0 && (
          <p className="rounded-2xl border border-dashed border-ink-100 p-8 text-center text-sm text-ink-400 dark:border-ink-700">
            No trips yet. Start with the AI Trip Planner.
          </p>
        )}
