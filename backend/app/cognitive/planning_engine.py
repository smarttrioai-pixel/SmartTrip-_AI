"""
  ...
  Orchestrates trip itinerary generation through the full SCIF pipeline:
      ...
      LLMService        (orchestration: retries, logging, circuit breaker)
           ↓
      GroqProvider      (Groq Inference API → configured Qwen model)
...
class PlanningEngine:
    ...
    async def generate_plan(self, user_id: str, request: GenerateItineraryRequest) -> RawPlan:
        ...
        # Deterministic post-processing: the LLM may propose activities, but
        # transport assumptions and chronological order are validated locally.
        effective_transport = (
            request.transport
        ...
        # Resolve meal activities to real restaurants before the itinerary is
        # persisted/returned. Qwen supplies cuisine/food intent; OpenTripMap
        # supplies the actual place metadata. The LLM is never allowed to
        # become the source of truth for restaurant names, ratings, images,
        ...
        for day in days:
            for activity in day.get("activities", []):
                ...
                if category != "meal" and not (meal_type or food_query):
                    continue

                try:
                    enriched = await self._place_enrichment.enrich_place(
                        title=str(activity.get("title") or "Meal"),
                    ...
                    logger.warning(
                        "Meal place enrichment failed for %r in %r: %s",
                        activity.get("title"), request.destination, exc,
                    )
                    enriched = None

                if enriched and enriched.get("matched_place_name"):
                    activity["title"] = f"{str(meal_type).title()} at {enriched['matched_place_name']}" if meal_type else enriched["matched_place_name"]
