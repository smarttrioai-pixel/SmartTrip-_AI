...
Orchestrates trip itinerary generation through the full SCIF pipeline:
    ...
    LLMService        (orchestration: retries, logging, circuit breaker)
         ↓
    BaseLLMProvider   (HuggingFaceProvider by default → Gemma model)
...
stack. Switching AI providers requires only changing LLM_PROVIDER — no
changes to this engine, no changes to any route.

The SCIF Cognitive Engines (Memory, Recommendation, Explainability, Risk,
Context, UserProfile) are all preserved and continue to operate exactly
as before.
"""
...
class PlanningEngine:
    ...
    async def generate_plan(self, user_id: str, request: GenerateItineraryRequest) -> RawPlan:
        ...
        # generate_json() handles validation, retry on parse failure,
        # circuit breaker, and structured logging. No Gemini call here.
        # ----------------------------------------------------------------
