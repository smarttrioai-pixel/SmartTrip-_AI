...
Phase 5 (Retrieval Engine) changes:
  - get_context() now delegates vector search to RetrievalEngine instead
    of doing its own embed_text + cosine_similarity loop.  The engine
    returns a StructuredContext whose .as_prompt_text() is the drop-in
    replacement for the old .as_prompt_context().
  - save_preference() calls retrieval_engine.index_user_memory() after
    persisting to Firestore so the memory index stays in sync.
  - retrieval_engine is an optional constructor parameter: if None is
    passed (legacy / test callers), the old inline cosine path is used
    as a fallback — backward compatible.
...
class MemoryEngine:
    def __init__(
        ...
        retrieval_engine=None,  # app.engines.RetrievalEngine | None
    ) -> None:
        self._memory = memory_repository
        self._chats = chat_repository
        self._retrieval = retrieval_engine  # None → legacy cosine path
    ...
    async def get_context(
        ...
        *,
        ...
        if self._retrieval is not None and longterm.embeddings:
            # --- Phase 5 path: delegate to RetrievalEngine ---
            try:
                # Ensure the memory index is up to date for this user
                await self._retrieval.index_user_memory(user_id, longterm)
                result = await self._retrieval.retrieve_user_memory(
                    user_id, current_request_text, top_k=TOP_K_PREFERENCES
                )
                # Map back to PreferenceEmbedding for downstream compatibility
                doc_texts = {e.source_text for e in longterm.embeddings}
                relevant_preferences = [
                    e
                    for e in longterm.embeddings
                    if e.source_text in {d.content for d in result.documents}
                ]
                logger.debug(
                    "MemoryEngine.get_context | RetrievalEngine path | "
                    "retrieved=%d prefs | user=%s",
                    len(relevant_preferences), user_id,
                )
            except Exception as exc:
                logger.warning(
                    "MemoryEngine.get_context | RetrievalEngine failed (%s) — "
                    "falling back to direct cosine path", exc
                )
                relevant_preferences = self._legacy_cosine_search(
                    longterm.embeddings, current_request_text=None, query_vector=None
                )
        elif longterm.embeddings:
            # --- Legacy path: direct embed + cosine (used when no engine injected) ---
            ...
            relevant_preferences = [
                e for score, e in scored[:TOP_K_PREFERENCES]
    ...
    @staticmethod
    def _legacy_cosine_search(
        embeddings: list[PreferenceEmbedding],
        current_request_text: str | None,
        query_vector: list[float] | None,
    ) -> list[PreferenceEmbedding]:
        """Fallback: pure in-memory cosine search (no RetrievalEngine)."""
        ...
        scored.sort(key=lambda pair: pair[0], reverse=True)
        return [e for score, e in scored[:TOP_K_PREFERENCES] if score >= SIMILARITY_THRESHOLD]
    ...
    async def save_preference(self, user_id: str, source_text: str, source_type: str) -> None:
        ...
        # --- Phase 5: keep memory index in sync after every preference save ---
        if self._retrieval is not None:
            try:
                await self._retrieval.index_user_memory(user_id, longterm)
            except Exception as exc:
                logger.warning(
                    "MemoryEngine.save_preference | index_user_memory failed (%s) — "
                    "preference saved to Firestore but memory index not updated", exc
                )
