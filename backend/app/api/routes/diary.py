...
the requesting user) and calls Gemini with the trip's actual itinerary
data, instead of returning a templated fake journal with a fixed date
and a fixed $145.50 expense figure regardless of the trip. /export-pdf
previously hand-assembled a fake PDF byte string (not using any PDF
library, content was just the trip_id) — replaced with an honest plain-
text export. Real PDF generation (e.g. via reportlab/weasyprint) is
out of scope for this pass; this is flagged, not silently left mislabeled
as a PDF that isn't one.
"""
...
from app.api.deps import CurrentUser, get_trip_repository
...
async def generate_diary_entry(
    ...
    try:
        ai_result = await generate_json(system_prompt=DIARY_SYSTEM_PROMPT, user_prompt=user_prompt)
