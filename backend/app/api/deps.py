...
def get_llm_provider() -> BaseLLMProvider:
    """
    Construct the active LLM provider from LLM_PROVIDER config.

    Returns a singleton-per-process provider instance. The provider type
    is selected from settings.LLM_PROVIDER — no code change is needed
    when switching providers, only the environment variable.
    """
    ...
    if provider_name == "huggingface":
        ...
        if not _settings.HF_API_TOKEN:
            raise RuntimeError(
                "LLM_PROVIDER=huggingface but HF_API_TOKEN is not set. "
                "Set HF_API_TOKEN in your environment or .env file."
            )
        return HuggingFaceProvider(
            api_token=_settings.HF_API_TOKEN,
            model=_settings.HF_MODEL,
        )
    ...
    raise RuntimeError(
        ...
        "Supported values: 'huggingface', 'gemini'."
    )
...
def get_llm_service(
    ...
    """Construct LLMService with the active provider."""
...
def get_embedding_service() -> EmbeddingService:
    ...
    is separate from the generative model (HF_MODEL). Embeddings are always
    served by HuggingFace regardless of LLM_PROVIDER setting.
    """
