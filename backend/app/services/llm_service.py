...
routes, services) and the provider layer (HuggingFace, Gemini, etc.).

Responsibilities:
  - Provider selection from LLM_PROVIDER config
  - Prompt construction
...
BaseLLMProvider. Switching providers requires only changing LLM_PROVIDER.
"""
...
class LLMService:
    ...
    The provider is chosen at startup from LLM_PROVIDER config — no code
