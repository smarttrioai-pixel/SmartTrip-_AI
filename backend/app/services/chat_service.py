...
(text generation via the active provider — HuggingFace by default).
...
Architecture:
    ...
    LLMService.chat()            ← Provider abstraction
         ↓
    BaseLLMProvider (HuggingFace / Gemma by default)

No direct Gemini calls. Switching providers requires only LLM_PROVIDER env var.
"""
...
class ChatService:
    ...
    async def send_message(self, user_id: str, request: SendMessageRequest) -> SendMessageResponse:
        ...
        # ------------------------------------------------------------------
        # LLMService → Provider → Model (HuggingFace by default)
        # ------------------------------------------------------------------
