...
class RetrievalEngine:
    ...
    async def index_user_memory(
        ...
        for pref in getattr(longterm_memory, "inferred_preferences", []):
            if getattr(pref, "status", "active") != "active":
                continue
            vector = await self._get_embedding(pref.statement)
            self._indexes.memory.add(
                text=pref.statement,
                vector=vector,
                metadata={
    ...
    async def index_trip(self, user_id: str, trip: Any) -> None:
        ...
        vector = await self._get_embedding(summary)
        self._indexes.memory.add(
            text=summary,
            vector=vector,
            metadata={
    ...
    async def index_research_document(
        ...
        if not text.strip():
            return
        vector = await self._get_embedding(text[:1000])  # cap at 1000 chars per chunk
    ...
    async def _get_embedding(self, text: str) -> list[float]:
        """
        Embed text with a two-tier cache check first.
        Falls back to a zero vector on error (logged as warning) to avoid
        crashing the entire retrieval pipeline on a transient API failure.
        """
        ...
        try:
            ...
            logger.warning(
                "RetrievalEngine: embedding failed for text=%r (%s) — returning zero vector",
                text[:60], exc,
            )
            vector = [0.0] * 768  # Gemini text-embedding-004 dimension
